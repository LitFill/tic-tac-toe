...
for (let baris = 0; baris <= 2; baris++) {
    for (let kolom = 0; kolom <= 2; kolom++) {
        objTombolPapan[`at${baris}${kolom}`] = document.querySelector(
            `.tombol-${baris}${kolom}`
        );
        objTombolPapan[`at${baris}${kolom}`].addEventListener("click", (e) => {
            if (e.target.innerText === "" && !pemenang) {
...
function cekPemenang() {
    ...
    for (let baris = 0; baris < 3; baris++) {
        if (
            ...
            for (let kolom = 0; kolom < 3; kolom++) {
                objTombolPapan[`at${baris}${kolom}`].style.color = "red";
            }
    ...
    if (
        ...
        for (let index = 0; index < 3; index++) {
            objTombolPapan[`at${index}${index}`].style.color = "red";
        }
    ...
    if (
        ...
        for (let index = 0; index < 3; index++) {
            objTombolPapan[`at${index}${2 - index}`].style.color = "red";
        }
